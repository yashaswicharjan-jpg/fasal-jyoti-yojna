...
const MyPostsTab = () => {
  ...
  const CATEGORY_META: Record<string, { label: string; emoji: string }> = {
    success_story: { label: t('community.success_story'), emoji: '🏆' },
    question: { label: t('community.question'), emoji: '❓' },
    tip: { label: t('community.tip'), emoji: '💡' },
    market_info: { label: t('community.market_info'), emoji: '💹' },
    weather_alert: { label: t('community.weather_alert'), emoji: '🌧️' },
  };
  ...
  const getLocale = () => i18n.language === 'hi' ? 'hi-IN' : i18n.language === 'mr' ? 'mr-IN' : 'en-IN';
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-3 gap-2">
        {[
          { label: t('profile.total_posts'), value: posts.length, icon: '📝', color: 'bg-primary/10 text-primary' },
          { label: t('profile.total_likes'), value: totalLikes, icon: '❤️', color: 'bg-destructive/10 text-destructive' },
          { label: t('profile.total_comments'), value: totalComments, icon: '💬', color: 'bg-accent/10 text-accent' },
        ].map(stat => (
          ...
          <motion.div key={post.id} layout>
            <GlassCard className="space-y-2">
              ...
              <div className="flex justify-end">
                ...
                >
                  <Trash2 size={12} />
                  {deleting === post.id ? t('profile.deleting') : t('community.delete_post')}
                </button>
              </div>
            </GlassCard>
          </motion.div>
